...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  init_len_pts3d = len(points3D)
  points3D = np.append(points3D, new_points3D, axis=0)

  for im_name, ls in corrs.items():
    for (offset, correspondances) in ls:
      images[im_name].Add3DCorrs(correspondances,list(range(offset + init_len_pts3d, init_len_pts3d + offset + len(correspondances))))
