...
def main():
  ...
  abs_pose = np.c_[np.eye(3),np.zeros(3)]
  print(abs_pose[:,0:3], abs_pose[:,3])
  e_im2.SetPose(abs_pose[:,0:3],abs_pose[:,3])
  ...
  for i in range(len(possible_relative_poses)):
    ...
    e_im1.SetPose(R,t)
    all,_,_ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    n = len(all)
    if max_pts < n:
      max_pts = n
      idx_rel_pos = i
      
  print("Posed picked: ",possible_relative_poses[idx_rel_pos])
  
  # Set the image poses in the images (image.SetPose(...))
  e_im1.SetPose(*possible_relative_poses[idx_rel_pos])
