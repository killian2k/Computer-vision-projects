...
def num_inlier(x,y,k,b,n_samples,thres_dist):
	...
	mask = (y-(np.dot(k,x)+b))**2 <= thres_dist
	num = mask.sum()
	return num, mask
...
def ransac(x,y,iter,n_samples,thres_dist,num_subset):
	...
	best_inliers = 0
	for _ in range(iter):
		subset = random.sample(range(n_samples),num_subset)
		x_p = x[subset]
		y_p = y[subset]
		k, b = least_square(x_p, y_p)
		n_inliers, mask = num_inlier(x,y,k,b,n_samples, thres_dist)
		if n_inliers > best_inliers:
			best_inliers = n_inliers
			k_ransac, b_ransac, inlier_mask = k, b, mask
