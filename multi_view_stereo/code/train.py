...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs, image_outputs = train_sample(sample, detailed_summary=do_summary, device=device)
            if do_summary:
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            global_step = len(TrainImgLoader) * epoch_idx + batch_idx
            do_summary = global_step % args.summary_freq == 0
            loss, scalar_outputs, image_outputs = test_sample(sample, detailed_summary=do_summary, device=device)
            if do_summary:
...
def train_sample(sample, detailed_summary=False, device='cpu'):
    model.train()
    optimizer.zero_grad()
...
@make_nograd_func
def test_sample(sample, detailed_summary=True, device="cpu"):
    model.eval()
